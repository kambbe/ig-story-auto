...
GitHubの `schedule` cron は**数時間遅れることがある**（混雑時に発火が遅延）。朝の時刻にきっちり出したいなら、
外部スケジューラ **cron-job.org** から `workflow_dispatch` を叩く構成にする（即時実行＝正確）。
GitHub側の cron はバックアップとして残す。手順は [運用ガイド.md](./運用ガイド.md) の
「正確な時刻で投稿する（cron-job.org）」を参照。

投稿の失敗（GitHubの一時的な503など）に備え、cron-job.org 側で数分ずらした予備トリガーを複数持たせ、
二重投稿ガードで1本に収める（同ガイド参照）。

## メモ

- ストーリーへのリンクスタンプはAPIでは付けられない（自動投稿では不可）。オンライン導線はプロフィールのリンクへ。
- 当日ぶんの営業変更は、スプレッドシートに**投稿時刻より前**に書いておく。
